...
class DataEncoder:
    ...
    def encode_action_histories(self, table):
        all_street_histories = [[player.round_action_histories[street] for player in table.seats.players] for street in
                                range(4)]
        past_street_histories = [histories for histories in all_street_histories if
                                 any([e is not None for e in histories])]
    ...
    def __order_histories(self, start_pos, player_histories):
        ordered_player_histories = [player_histories[(start_pos + i) % len(player_histories)] for i in
                                    range(len(player_histories))]
